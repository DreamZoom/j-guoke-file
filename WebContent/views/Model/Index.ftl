				...
				var table = $("#table").Table({
					columns: [
						...
						render: function(item, record) {
							...
							actions.push('<a data-command="editmodel" data-id="' + record.Id + '" class="label label-primary">编辑模型</a>');
				...
				$(document).on("click", "[data-command]", function() {
					...
					switch(command) {
						...
						case "editmodel":
							(function() {
								var id = $(context).data("id");
								window.location.href=CURRENT_PAGE_PATH+"/EditModel?projectId=${RequestParameters.projectId}&Id="+id;
//								var row = table.getRow(id);
//								editModalTemplate(row, function(form, modal) {
//									apiRequest("/Update", form, function() {
//										modal.modal("hide");
